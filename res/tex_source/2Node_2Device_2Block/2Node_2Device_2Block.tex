...
\begin{tikzpicture}
    \node (node1) {Node \#1};
    \node (node2) [right of=node1, node distance=10em] {Node \#2};
    \node (CPU) [below of=node1, node distance=4em] {GPU};
    \node (GPU1) [below of=node2, node distance=4em] {GPU \#1};
    ...
    \begin{pgfonlayer}{background}
        ...
        \path (block0.south -| block0.east)+(+1,-1) node (b) {};
        \path[fill=blue!20, draw=black!50, solid]
            (a) rectangle (b);

        \path (node2.north -| block1.west)+(-1, 0.5) node (a) {};
